...
@php
    ...
    $variant = is_object($variant ?? null) ? (array) $variant : ($variant ?? []);
...
@section('title', 'Editar variante - Admin')
...
<div class="admin-page-header">
    <div class="ui breadcrumb">
        ...
        <i class="right angle icon divider"></i>
        <div class="active section">Editar #{{ $variantId }}</div>
    </div>
...
<div class="admin-section">
        ...
        <div class="content">
            ...
            <div class="sub header">
                ...
                @if (!empty($variant['barcode']))
                    &middot; Código de barras: <code>{{ $variant['barcode'] }}</code>
                @endif
            </div>
        </div>
    </h2>
    ...
    <form class="ui form" method="POST" action="{{ route('admin.products.variants.update', ['product_id' => $productId, 'id' => $variantId]) }}">
        @csrf
        @method('PUT')
        ...
        <div class="form-section">
            ...
            <div class="two fields">
                <div class="field {{ $errors->has('name') ? 'error' : '' }}">
                    ...
                    <input type="text" name="name" value="{{ old('name', $variant['name'] ?? '') }}" required maxlength="255">
                </div>
                <div class="field {{ $errors->has('variant_type') ? 'error' : '' }}">
                    ...
                    <select name="variant_type" class="ui search dropdown" required>
                        ...
                        @foreach ($variantTypes as $value => $label)
                            <option value="{{ $value }}"
                                {{ old('variant_type', $variant['variant_type'] ?? '') === $value ? 'selected' : '' }}>{{ $label }}</option>
                        @endforeach
                    </select>
                </div>
            </div>
            <div class="two fields">
                <div class="field {{ $errors->has('sku') ? 'error' : '' }}">
                    ...
                    <input type="text" name="sku" value="{{ old('sku', $variant['sku'] ?? '') }}" required maxlength="255">
                </div>
                <div class="field {{ $errors->has('barcode') ? 'error' : '' }}">
                    <label>Código de barras <span style="color:#db2828;">*</span></label>
                    <input type="text" name="barcode" value="{{ old('barcode', $variant['barcode'] ?? '') }}" required maxlength="255">
                </div>
            </div>
            <div class="field {{ $errors->has('image_url') ? 'error' : '' }}">
                <label>URL da imagem</label>
                <input type="text" name="image_url" value="{{ old('image_url', $variant['image_url'] ?? '') }}" maxlength="1024">
            </div>
        </div>
            ...
            <div class="four fields">
                <div class="field {{ $errors->has('price_adjustment') ? 'error' : '' }}">
                    ...
                    <input type="number" step="0.01" name="price_adjustment"
                        value="{{ old('price_adjustment', $variant['price_adjustment'] ?? 0) }}">
                </div>
                <div class="field {{ $errors->has('stock') ? 'error' : '' }}">
                    <label>Estoque</label>
                    <input type="number" min="0" name="stock" value="{{ old('stock', $variant['stock'] ?? 0) }}">
                </div>
                <div class="field {{ $errors->has('reserved_stock') ? 'error' : '' }}">
                    <label>Estoque reservado</label>
                    <input type="number" min="0" name="reserved_stock"
                        value="{{ old('reserved_stock', $variant['reserved_stock'] ?? 0) }}">
                </div>
                <div class="field {{ $errors->has('min_stock') ? 'error' : '' }}">
                    <label>Estoque mínimo</label>
                    <input type="number" min="0" name="min_stock"
                        value="{{ old('min_stock', $variant['min_stock'] ?? 0) }}">
                </div>
            </div>
        </div>
        ...
        <div class="form-section">
            ...
            <div class="four fields">
                ...
                <div class="field">
                    <label>Altura</label>
                    <input type="number" step="0.01" min="0" name="dimensions[altura]"
                        value="{{ old('dimensions.altura', $dimensions['altura'] ?? '') }}">
                </div>
                <div class="field">
                    <label>Largura</label>
                    <input type="number" step="0.01" min="0" name="dimensions[largura]"
                        value="{{ old('dimensions.largura', $dimensions['largura'] ?? '') }}">
                </div>
                <div class="field">
                    <label>Profundidade</label>
                    <input type="number" step="0.01" min="0" name="dimensions[profundidade]"
                        value="{{ old('dimensions.profundidade', $dimensions['profundidade'] ?? '') }}">
                </div>
            </div>
        </div>
        ...
        <div class="form-section">
            ...
            <div class="three fields">
                <div class="field {{ $errors->has('order') ? 'error' : '' }}">
                    <label>Ordem</label>
                    <input type="number" min="0" name="order" value="{{ old('order', $variant['order'] ?? 0) }}">
                </div>
                <div class="field">
                    <label>&nbsp;</label>
                    <div class="ui toggle checkbox">
                        ...
                        <input type="checkbox" name="active" id="active" value="1"
                            {{ old('active', $variant['active'] ?? 1) ? 'checked' : '' }}>
                ...
                <div class="field">
                    ...
                    <div class="ui toggle checkbox">
                        ...
                        <input type="checkbox" name="is_default" id="is_default" value="1"
                            {{ old('is_default', $variant['is_default'] ?? 0) ? 'checked' : '' }}>
        ...
        <div class="form-actions">
            ...
            <a href="{{ route('admin.products.variants.index', ['product_id' => $productId]) }}" class="ui button">Voltar</a>
        </div>
    </form>
</div>
@endsection
